import os
import uuid # generating unique database names
import psycopg2
...
# Import your agent's main function
...
app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = 'Uploads' # tells flask where to store .sql files
app.config['SECRET_KEY'] = os.urandom(24)  # Required for Flask sessions
...
def get_db_admin_connection():
    ...
    try:
        ...
        # CRITICAL: Set autocommit to run CREATE DATABASE
...
def create_db_from_sql(sql_filepath):
    ...
    """
    admin_conn = get_db_admin_connection() # To connect to the main PostgreSQL server  with administrator privileges.It's like an object 
    if not admin_conn:
        return None
    
    cursor = admin_conn.cursor() # cursor is what you actually use to run SQL commands (queries) on the database.
    new_db_name = f"user_db_{uuid.uuid4().hex[:10]}" # Create a unique name
    ...
    # 2. Connect to the NEW database to load the schema
    try:
        ...
        # 3. Read and execute the user's .sql file
        ...
        # If population fails, we should drop the new DB to clean up
        # (This requires re-connecting as admin, logic omitted for brevity)
        return None
    finally:
...
def index():
    ...
    # Check if a database is already in the session
...
def upload_file():
    ...
    if file and file.filename.endswith('.sql'):
        ...
        if new_db_name:
            # SUCCESS! Save the new DB info in the user's session
